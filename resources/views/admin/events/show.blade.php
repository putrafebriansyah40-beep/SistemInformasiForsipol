        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-gradient-to-r from-secondary-600 to-secondary-800 rounded-2xl shadow-sm text-white p-6 relative overflow-hidden">
                ...
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <h3 class="text-2xl font-bold mb-1">{{ $event->nama_kegiatan }}</h3>
                        <p class="text-secondary-100">{{ \Carbon\Carbon::parse($event->waktu_pelaksanaan)->format('l, d M Y - H:i') }} WIB • {{ $event->lokasi ?? 'Lokasi tidak ditentukan' }}</p>
                    </div>
                    
                    <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/20 text-center min-w-[200px]">
                        <p class="text-xs text-secondary-100 uppercase tracking-wider mb-1">Kode Presensi</p>
                        <p class="text-3xl font-mono font-bold tracking-widest">{{ $event->kode_absen ?? '-' }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-4 border-t border-white/20 relative z-10">
                    ...
                    <p class="text-sm leading-relaxed">{{ $event->deskripsi }}</p>
                </div>
                @endif
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                    <h3 class="font-semibold text-gray-800">Daftar Hadir ({{ $event->attendances->count() }} Anggota)</h3>
                </div>
